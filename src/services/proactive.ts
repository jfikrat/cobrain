...
async function handleDailySummaryTask(task: QueuedTask): Promise<TaskResult> {
  ...
  try {
    ...
    let message = `<b>${t("daily.greeting")}</b>\n\n`;

    if (reminders.length > 0) {
      message += `<b>${t("daily.today_reminders")}</b>\n`;
      ...
      if (todayReminders.length === 0) {
        message += `  <i>${t("daily.no_reminders")}</i>\n`;
      }
      message += "\n";
    }

    message += `<b>${t("daily.pending_count")}</b> ${pendingCount}\n`;
    message += `\n<i>${t("daily.goodbye")}</i>`;
...
async function handleReminderTask(task: QueuedTask): Promise<TaskResult> {
  ...
  try {
    ...
    const response = await think(
      task.userId,
      t("daily.reminder_trigger", { action: actionText })
    );
